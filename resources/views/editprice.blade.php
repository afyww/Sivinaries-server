        ...
        <div class="p-6">
            <div class='w-full bg-white rounded-xl h-fit mx-auto'>
                <div class="p-3 text-center">
                    <h1 class="font-extrabold text-3xl">Edit price</h1>
                </div>
                <div class="p-6">
                    <form class="space-y-3" method="post" action="{{ route('updateprice', ['id' => $price->id]) }}"
                        ...
                        <div class="grid grid-cols-1">
                            <div class="space-y-2">
                                ...
                                <input type="text"
                                    ...
                                    id="service" name="service" value="{{ $price->service }}" required>
                            </div>
                        </div>
                                ...
                                <select name="user_id" id="user_id"
                                    ...
                                    @foreach ($user as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $price->user_id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="project_id" id="project_id"
                                    ...
                                    @foreach ($project as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $price->project_id ? 'selected' : '' }}>
                                            {{ $item->project }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1">
                            <div class="space-y-2">
                                ...
                                <input type="number"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 p-2 rounded-lg w-full"
                                    id="price" name="price" value="{{ $price->price }}" required>
                            </div>
                        </div>
